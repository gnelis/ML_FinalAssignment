...
The training set contains `r nfeats` columns and  `r nobs_training` observations. The testing set is smaller, with only `r nobs_testing` observations. To test the accuracy of our prediction, we will use a validation set extracted from the training set, and we will test the final performance on the testing set.
...
Now the dataset has `r dim(training)[1]` observations and `r dim(training)[2]` columns.
...
```{r mfit, cache=TRUE, message = FALSE}
library(parallel)
library(doParallel)
